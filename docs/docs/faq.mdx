...
Yes. Set the provider base-URL environment variable or pass `base_url`/`ProviderCredentials`. Custom endpoints and headers are trusted application configuration, but base URLs must use HTTPS and cannot contain userinfo, a query, or a fragment. HTTP is allowed only for the exact host `localhost` or a literal loopback IP such as `127.0.0.1` or `::1`; hostnames that resolve to loopback do not qualify. This is intentional hardening. Anthropic additionally requires its provider-returned results URL to match the configured API origin.
