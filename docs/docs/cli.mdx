...
The installed `batchwork` command operates one provider-native batch at a time. It exposes the same canonical text, embedding, and image request models as the Python API; it is not a scheduler or multi-batch workflow engine.
...
For an approved one-shot invocation:
...
Global controls are root-only and must precede the command:
