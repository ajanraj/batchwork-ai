---
title: Groq
description: 'Groq file-backed chat batches in Batchwork with OpenAI-compatible result files and the unified typed job lifecycle.'
---
...
Exact CLI text provider-option keys are `parallelToolCalls`, `reasoningEffort`, `reasoningFormat`, `serviceTier`, and `user`. Only `chat-completions` is a valid explicit `--endpoint`. Unknown keys fail locally; `parallelToolCalls` must be boolean and the remaining keys are strings. Canonical `top_k` is unsupported. Batch metadata is supported and forwarded. Package hard bounds are 50,000 requests, 20 MiB per serialized request, and 200 MiB aggregate, subject to lower Groq/model limits.
