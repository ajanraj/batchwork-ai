...
Mistral does not support canonical `dimensions`. Python keeps its prior non-strict behavior for unknown Mistral embedding provider options; CLI submission rejects them before upload.
