...
Batch-level `metadata=` is rejected before provider work. Anthropic request-level user metadata remains available through provider options.
...
Submission-level batch metadata is unsupported and rejected before provider work. Anthropic text batches retain the package-wide 50,000-request, 20 MiB/request, and 200 MiB aggregate bounds unless a lower provider/model limit is known.
