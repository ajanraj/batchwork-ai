...
Exact image option keys are `aspect_ratio`, `output_format`, `quality`, `resolution`, `sync_mode`, and `user`. Provider `aspect_ratio` collides with canonical `aspect_ratio`. Generic image `seed` and `size` are unsupported. Unknown keys and invalid values fail local preflight. CLI batch metadata is unsupported. Package hard bounds are 50,000 requests, 20 MiB per request, and 200 MiB aggregate, subject to lower xAI/model limits.
