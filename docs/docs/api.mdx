...
`DefaultMediaResolver` implements the default HTTPS and inline-media policy. Applications may inject a `MediaResolver`; successful resolutions return `ResolvedMedia`. `ProviderFileReference` and tagged file-data types represent provider-owned or explicitly typed media sources. See [Security](/docs/guides/security#remote-media) for SSRF, redirect, and size boundaries.
